...
/**
 * ExternalSchemas新建、编辑
 * @author hrj
 *
 */
public class ExternalSchemasCreateEditor extends MultiPageEditorParent {

	private EcoreExternalSchemaDao ecoreExternalSchemaDao = new EcoreExternalSchemaDaoImpl();
	...
	private Button technicalButton,btBtn,addNamespaceListBtn,deleteNamespaceListBtn,addExampleBtn,deleteExampleBtn,addConstrainBtn,editConstrainBtn,deleteConstraintBtn;

	private List namespaceList, exampleList;
	...
	private ArrayList<EcoreExternalSchema> externalSchemaNextVersionList = new ArrayList<>();
	private ArrayList<EcoreMessageComponent> ecoreMessageComponentList = new ArrayList<>();
	private ArrayList<EcoreMessageDefinition> ecoreMsgDefinitonList = new ArrayList<>();
	private ArrayList<EcoreMessageSet> ecoreMessageSetList = new ArrayList<>();
	...
	private void initFilledData() {
		// 不为null代表是打开的,可编辑
		try {
			ecoreExternalSchemaVO = ecoreExternalSchemaDao.findExternalSchemaVO(externalSchemaId);
			if (ecoreExternalSchemaVO != null) {
				ecoreMessageComponentList = DerbyDaoUtil.getMessageComponentListByExternalSchemaId(externalSchemaId);
				ecoreMsgDefinitonList = DerbyDaoUtil.getMessageDefinitionList(ecoreMessageComponentList);
				ecoreMessageSetList = DerbyDaoUtil.getMessageSetList(ecoreMsgDefinitonList);
				previousEcoreExternalSchema = DerbyDaoUtil.getPreviousVersionExternalSchema(externalSchemaId);
				externalSchemaNextVersionList = DerbyDaoUtil
						.getEcoreExternalSchemaListByIds(DerbyDaoUtil.getNextVersionIds(externalSchemaId));
				return;
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	protected void createPages() {
		...
		if (ecoreExternalSchemaVO.getEcoreExternalSchema() != null) {
			...
			if (RegistrationStatus.Registered.getStatus().equals(ecoreExternalSchemaVO.getEcoreExternalSchema().getRegistrationStatus())) {
				this.setDirty(false);
			} else {
				this.setDirty(true);
			}
	...
	private void createSummaryPage() {
		...
		ScrolledComposite sc = new ScrolledComposite(parentComposite, SWT.V_SCROLL | SWT.H_SCROLL);
		Composite parent = new Composite(sc, SWT.NONE);
		sc.setContent(parent);
		sc.setExpandHorizontal(true);
		sc.setExpandHorizontal(true);
		sc.setBounds(0, 0, width, height);
		sc.setLayoutData(new GridData(width, height));

		parent.setLayout(new GridLayout(2, true));
		parent.setLayoutData(new GridData(width, height));
		parent.setBounds(0, 0, width, height);

		int index = addPage(sc);
		setPageText(index, "Summary");

		addSummaryPageLeftComposite(parent);
		addSummaryPageRightComposite(parent);
	...
	private void addGeneralInformation(ExpandBar summaryBar) {
		...
		technicalButton = new Button(generalInformationComposite, SWT.CHECK);
...
//			btText.setText(ecoreExternalSchemaVO.getEcoreExternalSchema().get);
		...
		btBtn = new Button(generalInformationComposite, SWT.NONE);
		...
		btBtn.addMouseListener(new MouseAdapter() {
			...
			public void mouseUp(MouseEvent e) {
				try {
					setDirty(true);
					MessageBoxUtil.createSelectBusinessComponentDialog(btText);
				} catch (Exception e1) {
					e1.printStackTrace();
				}
			}
		});
		Button btSearchBtn = new Button(generalInformationComposite, SWT.NONE);
		...
		generalInformationItem.setHeight(270);
	...
	private void addNameSpaceList(ExpandBar summaryBar) {
		...
		Label exampleDescLabel = new Label(namespaceListComposite, SWT.WRAP);
		exampleDescLabel.setText("In this section, you can define namespaces.");
		exampleDescLabel.setBounds(10, 2, width, 15);

		addNamespaceListBtn = new Button(namespaceListComposite, SWT.NONE);
		addNamespaceListBtn.setImage(ImgUtil.createImg(ImgUtil.ADD_IMG_TOOLBAR));
		addNamespaceListBtn.setBounds(10, 20, 25, 25);
		addNamespaceListBtn.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseUp(MouseEvent e) {
				MessageBoxUtil.createNamespaceListDialogue(namespaceList);
				setDirty(true);
			}
		});

		deleteNamespaceListBtn = new Button(namespaceListComposite, SWT.NONE);
		deleteNamespaceListBtn.setBounds(40, 20, 25, 25);
		deleteNamespaceListBtn.setImage(ImgUtil.createImg(ImgUtil.DELETE_IMG_YES));
		deleteNamespaceListBtn.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseUp(MouseEvent e) {
				MessageBoxUtil.deleteExampleDialogue(namespaceList);
				setDirty(true);
			}
		});
		...
		ExpandItem namespaceListItem = new ExpandItem(summaryBar, SWT.NONE);
		namespaceListItem.setText("Namespace List");
		namespaceListItem.setHeight(160);
		namespaceListItem.setExpanded(true);
		namespaceListItem.setControl(namespaceListComposite);
	...
	private void addCMPInformation(ExpandBar summaryBar) {
		...
		ExpandItem cmpInformationItem = new ExpandItem(summaryBar, SWT.NONE);
		cmpInformationItem.setText("CMP Information");
		cmpInformationItem.setHeight(65);
	...
	private void addRegistrationInformation(ExpandBar summaryBar) {
		...
		desLb.setText("Registration Information");
		desLb.setBounds(10, 2, width, 15);
		...
		rmCombo.add(RegistrationStatus.Registered.getStatus());
		rmCombo.add(RegistrationStatus.Provisionally.getStatus());
		rmCombo.add(RegistrationStatus.Added.getStatus());
		...
		ExpandItem generalInformationItem = new ExpandItem(summaryBar, SWT.NONE);
		generalInformationItem.setText("Registration Information");
		generalInformationItem.setHeight(90);
		generalInformationItem.setExpanded(true);
		generalInformationItem.setControl(generalInformationComposite);
	}
	...
	private void addExamples(ExpandBar summaryBar) {
		...
		Label exampleDescLabel = new Label(generalInformationComposite, SWT.WRAP);
		exampleDescLabel.setText("In this section, you can define examples.");
		exampleDescLabel.setBounds(10, 2, width, 15);

		addExampleBtn = new Button(generalInformationComposite, SWT.NONE);
		addExampleBtn.setImage(ImgUtil.createImg(ImgUtil.ADD_IMG_TOOLBAR));
		addExampleBtn.setBounds(10, 20, 25, 25);
		...
		deleteExampleBtn = new Button(generalInformationComposite, SWT.NONE);
		deleteExampleBtn.setBounds(40, 20, 25, 25);
		...
		exampleList = new List(generalInformationComposite, SWT.BORDER | SWT.H_SCROLL | SWT.V_SCROLL | SWT.MULTI);
		exampleList.setBounds(10, 50, width / 2 - 70, 100);
		...
		ExpandItem generalInformationItem = new ExpandItem(summaryBar, SWT.NONE);
		generalInformationItem.setText("Examples");
		generalInformationItem.setHeight(150);
		generalInformationItem.setExpanded(true);
		generalInformationItem.setControl(generalInformationComposite);
	}
	...
	private void addConstraints(ExpandBar summaryBar) {
		...
		Label contraintsDescLabel = new Label(generalInformationComposite, SWT.WRAP);
		...
		contraintsDescLabel.setBounds(10, 10, (width - 150) / 2, 40);

		addConstrainBtn = new Button(generalInformationComposite, SWT.NONE);
		addConstrainBtn.setBounds(10, 50, 25, 25);
		addConstrainBtn.setImage(ImgUtil.createImg(ImgUtil.ADD_IMG_TOOLBAR));
		addConstrainBtn.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseUp(MouseEvent e) {
				setDirty(true);
				MessageBoxUtil.createConstraintDialogue("add", "1", constraintsTable, null, dataType);
			}
		});

		editConstrainBtn = new Button(generalInformationComposite, SWT.NONE);
		editConstrainBtn.setBounds(40, 50, 25, 25);
		editConstrainBtn.setImage(ImgUtil.createImg(ImgUtil.EDIT_IMG_TOOLBAR));
		editConstrainBtn.addMouseListener(new MouseAdapter() {
			...
			public void mouseUp(MouseEvent e) {
				...
				if (constraintsTable.getSelectionCount() > 0) {
					setDirty(true);
					MessageBoxUtil.createConstraintDialogue("edit", "1", constraintsTable, null, dataType);
				}
			}
		});

		deleteConstraintBtn = new Button(generalInformationComposite, SWT.NONE);
		...
		deleteConstraintBtn.addMouseListener(new MouseAdapter() {
			...
			public void mouseUp(MouseEvent e) {
				setDirty(true);
				MessageBoxUtil.deleteConstraint("1", constraintsTable, null, dataType);
			}
		});
		...
		ExpandItem generalInformationItem = new ExpandItem(summaryBar, SWT.NONE);
		generalInformationItem.setText("Constraints");
		generalInformationItem.setHeight(220);
		generalInformationItem.setExpanded(true);
		generalInformationItem.setControl(generalInformationComposite);
	}
	...
	void createImpactPage() {
		...
		// 构造Message Set表格
		GenerateTableForMCandMDandMS.generateMessageSetTableOfImpactPage(messageSetTable, ecoreMessageSetList);
		...
		GenerateTableForMCandMDandMS.generateMessageComponentTableOfImpactPage(messageComponentTable,
		...
		messageSetTableCursor.addMouseListener(new MouseAdapter() {
			...
			public void mouseUp(MouseEvent e) {
				...
				GenerateCommonTree.generateContainmentTreeForMessageSetofImpactPage(treeRoot,
						DerbyDaoUtil.getMsgDefinitionByMsgSetId(messageSetId));
				// 加载Constraint节点
		...
		msgComponentTableCursor.addMouseListener(new MouseAdapter() {
			...
			public void mouseUp(MouseEvent e) {
				...
				GenerateCommonTree.generateContainmentTreeForMessageComponent(treeRoot,
						DerbyDaoUtil.getMessageElementList(msgComponentId));
				// 加载Constraint节点
	...
	public EcoreExternalSchemaVO getAllData() {
		...
		if (exampleItems != null && exampleItems.length > 0) {
			for (String str : exampleItems) {
				...
				bcExample.setObj_id(ecoreExternalSchema.getId());
				bcExample.setObj_type(ObjTypeEnum.ExternalSchema.getType());
