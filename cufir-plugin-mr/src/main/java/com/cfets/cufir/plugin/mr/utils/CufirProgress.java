 ...
 *
 */
public abstract class CufirProgress implements IRunnableWithProgress {
	...
	public CufirProgress(String taskName) {
		this(taskName, "", false);
	}

	public CufirProgress(String title, boolean percentable) {
		this(title, "", percentable);
	}

	public CufirProgress(String title, String note, boolean percentable) {
		this.title = title;
		this.note = note;
	 ...
	 */
	public void process() {
		CufirProgressMonitorDialog dialog = new CufirProgressMonitorDialog(title);
		try {
			// 运行
