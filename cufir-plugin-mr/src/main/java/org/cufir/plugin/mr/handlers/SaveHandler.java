...
import org.cufir.plugin.mr.editor.MessageHelper;
...
public class SaveHandler extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if ("dataTypesCreate".equals(name) && ier.isDirty()) {
			...
			if(countRepeat != null) {
				throw new RuntimeException("业务组件名称重复，请更新！");
			}
			try {
			...
			if(countRepeat != null) {
				throw new RuntimeException("组件名称重复，请更新！");
			}
			//删除版本关联
	...
	private String componentPreviousName(String name) {
		...
		String version = MessageHelper.getVersion(name, p, split);
		if("".equals(version)) {
			...
			if(version.length() <= 2) {
				...
				throw new RuntimeException("组件名称不规范,版本号最多只可以三位数字！");
			}
		}
		if(split.length == 1) {
	...
	private String definitionPreviousName(String name) {
		...
		if("".equals(version)) {
			...
			if(version.length() == 3) {
				...
				throw new RuntimeException("报文名称不规范,版本号只可以两位数字！");
			}
		}
		if(split.length == 1) {
			previous = split[0] + previousVersion;
