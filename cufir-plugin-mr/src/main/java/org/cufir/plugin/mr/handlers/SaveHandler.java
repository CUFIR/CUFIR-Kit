...
public class SaveHandler extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		String name = ier.getPartProperty(MrHelper.SAVE_CUSTOM_NAME);
		// 左边的树的节点增加
		...
		if (TreeMenuEnum.DATA_TYPES.getName().equals(name) && ier.isDirty()) {
			saveDateType(ier,editorInput, treeListItem);
		} else if (TreeMenuEnum.BUSINESS_COMPONENTS.getName().equals(name) && ier.isDirty()) {
			saveBusinessComponent(ier,editorInput, treeListItem);
		} else if (TreeMenuEnum.EXTERNAL_SCHEMAS.getName().equals(name) && ier.isDirty()) {
			saveExternalSchema(ier,editorInput, treeListItem);
		} else if (TreeMenuEnum.MESSAGE_COMPONENTS.getName().equals(name) && ier.isDirty()) {
			saveMessageComponent(ier,editorInput, treeListItem);
		} else if (TreeMenuEnum.BUSINESS_AREAS.getName().equals(name) && ier.isDirty()) {
			saveBusinessArea(ier,editorInput, treeListItem);
		} else if (TreeMenuEnum.MESSAGE_SETS.getName().equals(name) && ier.isDirty()) {
			saveMessageSet(ier,editorInput, treeListItem);
		} else if (TreeMenuEnum.MESSAGE_DEFINITIONS.getName().equals(name) && ier.isDirty()) {
			saveMessageDefinition(ier,editorInput, treeListItem);
		}
		return null;
	}
	 ...
	 * @return
	 */
	private Object saveDateType(MultiPageEditorPart ier, MrEditorInput editorInput, MrTreeItem treeListItem) {
		...
		//名称重复验证
		if(repeatTips(treeItems, allData.getEcoreDataType().getName()) == null) {
			return null;
		}
		try {
	 ...
	 * @return
	 */
	private Object saveBusinessComponent(MultiPageEditorPart ier, MrEditorInput editorInput, MrTreeItem treeListItem) {
		BusinessComponentEditor bizEditor = (BusinessComponentEditor) ier;
		TreeItem[] treeItems = bizEditor.modelExploreTreeItem.getParentItem().getItems();
		...
		//名称重复验证
		if(repeatTips(treeItems, bcName) == null) {
			return null;
		}
		try {
		...
		ecoreTreeNodeDT.setName(bcName);
		setTreeItem(treeListItem, ecoreTreeNodeDT, editorInput.getTransferDataBean().getType(), "");
		// 保证新建和打开的只存在一个
		editorInput.getTransferDataBean().setId(ecoreTreeNodeDT.getId());
		editorInput.getTransferDataBean().setType(editorInput.getTransferDataBean().getType());
	 ...
	 * @return
	 */
	private Object saveExternalSchema(MultiPageEditorPart ier, MrEditorInput editorInput, MrTreeItem treeListItem) {
	 ...
	 * @return
	 */
	private Object saveMessageComponent(MultiPageEditorPart ier, MrEditorInput editorInput, MrTreeItem treeListItem) {
		...
		//名称重复验证
		if(repeatTips(treeItems, msgComponentName) == null) {
			return null;
		}
		String pat = "\\d+";
		...
		try {
			MrImplManager.get().getEcoreMessageComponentImpl().saveMessageComponentVOList(Arrays.asList(msgComponentVO));
		} catch (Exception e) {
			e.printStackTrace();
		}
		//多线程加载
	 ...
	 * @return
	 */
	private Object saveBusinessArea(MultiPageEditorPart ier, MrEditorInput editorInput, MrTreeItem treeListItem) {
	 ...
	 * @return
	 */
	private Object saveMessageSet(MultiPageEditorPart ier, MrEditorInput editorInput, MrTreeItem treeListItem) {
	 ...
	 * @return
	 */
	private Object saveMessageDefinition(MultiPageEditorPart ier, MrEditorInput editorInput, MrTreeItem treeListItem) {
	 ...
	 * @return
	 */
	public Object repeatTips(TreeItem[] treeItems, String name) {
		Map<String, Integer> repeat = repeat(treeItems);
		Integer countRepeat = repeat.get(name);
		if(countRepeat != null) {
			MessageDialog.openWarning(Display.getDefault().getActiveShell(), I18nApi.get("tips.name"), I18nApi.get("tips.error.name.repeat"));
			return null;
		}
		return "0";
	}
