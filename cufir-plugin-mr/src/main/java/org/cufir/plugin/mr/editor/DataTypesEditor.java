...
//		createVersionSubset();
	...
	private void createSummaryPage() {
		...
		if (ecoreDataTypeVO.getEcoreDataType() != null) {
			if (DataTypesEnum.CODE_SETS.getType().equals(ecoreDataTypeVO.getEcoreDataType().getType())) { // 是Code Set，并且是 可衍生的: 那么可编辑
				if(ecoreDataTypeVO.getEcoreDataType().getTrace() == null && RegistrationStatusEnum.Registered.getStatus().equals(ecoreDataTypeVO.getEcoreDataType().getRegistrationStatus())) {
					isEditable = true;
				} else {
					isEditable = false;
				}
			}else {
				if(!RegistrationStatusEnum.Registered.getStatus().equals(ecoreDataTypeVO.getEcoreDataType().getRegistrationStatus())
						&& !RegistrationStatusEnum.Obsolete.getStatus().equals(ecoreDataTypeVO.getEcoreDataType().getRegistrationStatus())) {
					isEditable = true;
				}else {
					isEditable = false;
				}
	...
	private void createContentPageForCodeSets() {
		...
		if (ecoreDataTypeVO.getEcoreDataType() != null 
				&& (RegistrationStatusEnum.Registered.getStatus().equals(ecoreDataTypeVO.getEcoreDataType().getRegistrationStatus()))) {
