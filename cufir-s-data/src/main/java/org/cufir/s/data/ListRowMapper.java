	...
	public T mapRow(ResultSet rs, int index) throws SQLException {
		...
		try {
			...
			for (TableColumnModel tcm : tcms) {
				...
				// @since 2.0.4.0 Long型有问题，对应Mysql是Int(10)<->java.math.BigInt
				String javatype = tcm.getJavatype();
				// @since 3.2.1.0 将 if else 修改成 switch @author TT
				switch (javatype) {
				case DATA_TYPE_LONG:
				...
				case DATA_TYPE_BIGDECIMAL:
					...
					// 增加对Format的支持 @since 2.1.6.0 ,并处理格式化异常@since2.1.7.0
					try {
				...
				case DATA_TYPE_BOOLEAN:
					...
					break;
				case DATA_TYPE_DATE:
					// 增加Date字段支持 @since 2.1.7.0 并处理格式化异常
