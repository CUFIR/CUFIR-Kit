 ...
 *
 */
public class EcoreExternalSchemaDaoImpl implements EcoreExternalSchemaDao{

	private static Logger logger = Logger.getLogger(EcoreExternalSchemaDaoImpl.class);
	...
	public EcoreTreeNode findEcoreExternalSchemaTreeNodes() throws Exception{
		...
		String esql = "SELECT T.ID,T.NAME,t.registration_status FROM ECORE_EXTERNAL_SCHEMA T ORDER BY T.NAME";
		...
		try {
			...
			//顶级
			ecoreTreeNode.setName("External Schemas");
			ecoreTreeNode.setLevel("1");
			ecoreTreeNode.setType("3");
			ecoreTreeNode.setChildNodes(externalSchemas);
			conn.commit();
		} catch (Exception e) {
			logger.error("更新数据失败!");
			conn.rollback();
